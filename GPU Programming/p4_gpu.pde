...
void draw() {
  ...
  for (int i = 0; i < 50; i++) {
    for (int j = 0; j < 50; j++) {
      ...
      beginShape();
      texture(sheep_texture);
      vertex(x + i * inc, y + j * inc, 0, i / uv_div, j / uv_div);
      vertex(x + (i + 1) * inc, y + j * inc, 0, (i + 1) / uv_div, j / uv_div);
      vertex(x + (i + 1) * inc, y + (j + 1) * inc, 0, (i + 1) / uv_div, (j + 1) / uv_div);
      vertex(x + i * inc, y + (j + 1) * 5.0, 0, i / uv_div, (j + 1) / uv_div);
      endShape();
      popMatrix();
    }
  }
